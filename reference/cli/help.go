...
func runHelp(cmd *cobra.Command, allCommands []*cobra.Command, args []string) {
	if len(args) == 0 {
		...
		for _, t := range []string{types.TypeApp, types.TypeExtension, types.TypeAuxiliary} {
			PrintHelpByTag(cmd, allCommands, t)
		}
		cmd.Println("Flags:")
		cmd.Println("  -h, --help   help for bdcctl")
		cmd.Println()
		cmd.Println(`Use "bdcctl [command] --help" for more information about a command.`)
	} else {
