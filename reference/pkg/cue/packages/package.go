...
import (
	"fmt"
	"kdp-oam-operator/reference/pkg/stdlib"
	"path/filepath"
	"strings"
	"sync"
	"time"
...
type PackageDiscover struct {
	kdpBuiltinPackages []*build.Instance
...
// ImportBuiltinPackagesFor will add kdp built-in packages into your CUE instance
func (pd *PackageDiscover) ImportBuiltinPackagesFor(bi *build.Instance) {
	pd.mutex.RLock()
	defer pd.mutex.RUnlock()
	bi.Imports = append(bi.Imports, pd.kdpBuiltinPackages...)
}
...
func (pd *PackageDiscover) mount(pkg *pkgInstance, pkgKinds []VersionKind) {
	...
	for i, p := range pd.kdpBuiltinPackages {
		if p.ImportPath == pkg.ImportPath {
			pd.pkgKinds[pkg.ImportPath] = pkgKinds
			pd.kdpBuiltinPackages[i] = pkg.Instance
			return
		}
	}
	pd.pkgKinds[pkg.ImportPath] = pkgKinds
	pd.kdpBuiltinPackages = append(pd.kdpBuiltinPackages, pkg.Instance)
}
