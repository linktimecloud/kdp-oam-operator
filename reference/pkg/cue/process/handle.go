...
import (
	...
	"encoding/json"
	"fmt"
	model2 "kdp-oam-operator/reference/pkg/cue/model"
	"reflect"
	"strings"
	"unicode"
...
type Context interface {
	SetBase(base model2.Instance) error
	AppendAuxiliaries(auxiliaries ...Auxiliary) error
	Output() (model2.Instance, []Auxiliary)
...
func NewContext(data ContextData) Context {
	...
	ctx.PushData(model2.ContextName, data.Name)
	ctx.PushData(model2.ContextNamespace, data.Namespace)
	ctx.PushData(model2.ContextWorkflowName, data.WorkflowName)
	ctx.PushData(model2.ContextPublishVersion, data.PublishVersion)
	return ctx
}

// SetParameters sets templateContext parameters
func (ctx *templateContext) SetParameters(params map[string]interface{}) {
	ctx.PushData(model2.ParameterFieldName, params)
}

// SetBase set templateContext base model
func (ctx *templateContext) SetBase(base model2.Instance) error {
...
func (ctx *templateContext) BaseContextFile() (string, error) {
	...
	if ctx.base != nil {
		...
		ctx.PushData(model2.OutputFieldName, b)
	}
	...
	if len(ctx.auxiliaries) > 0 {
		...
		if len(auxLines) > 0 {
			ctx.PushData(model2.OutputsFieldName, auxLines)
		}
	}
...
func (ctx *templateContext) BaseContextLabels() map[string]string {
	return map[string]string{
		model2.ContextName: fmt.Sprint(ctx.GetData(model2.ContextName)),
	}
}

// Output return model and auxiliaries of templateContext
func (ctx *templateContext) Output() (model2.Instance, []Auxiliary) {
...
func (ctx *templateContext) InsertSecrets(outputSecretName string, requiredSecrets []RequiredSecrets) {
	if outputSecretName != "" {
		ctx.PushData(model2.OutputSecretName, outputSecretName)
	}
